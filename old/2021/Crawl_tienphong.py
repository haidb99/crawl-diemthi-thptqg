...
def Crawler2021(ma_tinh, chrome_path, save_path, so_luong = 9999, start = -1, chrome_options = 0):
    ...
    # url crawl
    print("Watting....")
    url = 'https://tienphong.vn/tra-cuu-diem-thi.tpo'
    driver.get(url)
    ...
    while(1):
        ...
        if(len(list_node) > 0):
            for node in list_node:
                ...
                std = {}
                if(len(monhoc) == 10):
                    std['SBD'] = monhoc[0].text
                    std['Cum_thi'] = ma_tinh
                    std['Toan'] = monhoc[1].text
                    std['Ngu_van'] = monhoc[2].text
                    std['Ngoai_ngu'] = monhoc[3].text
                    std['Vat_ly'] = monhoc[4].text
                    std['Hoa_hoc'] = monhoc[5].text
                    std['Sinh_hoc'] = monhoc[6].text
                    std['KHTN'] = ''
                    std['Lich_su'] = monhoc[7].text
                    std['Dia_ly'] = monhoc[8].text
                    std['GDCD'] = monhoc[9].text
                    std['KHXH'] = ''
